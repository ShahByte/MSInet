...
def train_model(model, data, im, labels, data_name):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 15 == 0:  # Display the figure after every 15 epochs
            if args.visualize:
                seg_rgb = np.array([label_colours[c % 100] for c in seg_map])
                seg_rgb = seg_rgb.reshape(im.shape).astype(np.uint8)

                np.save(f'output/MSInet_seg_{data_name}_rgb_epoch_{epoch}.npy', seg_rgb)
                cv2.imwrite(f'output/MSInet_seg_{data_name}_rgb_epoch_{epoch}.png', seg_rgb)
        ...
        if nLabels <= args.minLabels:
            ...
            break  # This 'break' should be inside the 'if' block
    ...
    if args.visualize:
        ...
        plt.show()
        cv2.imwrite(f"output/MSInet_{data_name}_final.png", seg_map_out)
        np.savetxt(f"output/MSInet_{data_name}_final.txt", seg_map)
        np.save(f"output/MSInet_{data_name}_final.npy", seg_map_out)
